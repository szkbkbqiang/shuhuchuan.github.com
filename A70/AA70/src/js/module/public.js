    ...
    $public.prototype = {
        ...
        bingEvent: function () {
            ...
            $.getJSON(url + "?callback=?", body, function (data) {
                if (data.success) {
                    //                    ShowCountDownTime(data.result, "divdown1");

                    ShowCountDownTime(data.result);
                }
            });
        },
        alert: function (arr) {
        ...
        ShowCountDown: function (year, month, day, divname) {
            var _self = this;
            var now = new Date();
            //            var endDate = new Date('2016-09-21 20:00:00');
            var endDate = new Date(year, month - 1, day);
            var leftTime = endDate.getTime() - now.getTime();
            var leftsecond = parseInt(leftTime / 1000);
            //var day1=parseInt(leftsecond/(24*60*60*6)); 
...
var leftsecond = 0;
function ShowCountDownTime(nowtime) {
    var endtime = new Date('2016/9/21 20:00:00');
    var nowtime = new Date(nowtime);
    leftsecond = parseInt((endtime.getTime() - nowtime.getTime()) / 1000); //把剩余时间转化成秒。 
    if (parseInt(leftsecond / 3600 / 24) > 0) {
        $('.start-btn').click();
    }
    fresh()
    var sh;
...
function fresh() {
    leftsecond--;
    var d = parseInt((leftsecond - 60 * 60 * 4) / 3600 / 24);
    var h = parseInt(((leftsecond - 60 * 60 * 4) / 3600) % 24);
    var m = parseInt(((leftsecond - 60 * 60 * 4) / 60) % 60);
    var s = parseInt((leftsecond - 60 * 60 * 4) % 60);
    $("#divdown1").html("距离直播开始：" + todou1(d) + "天" + todou1(h) + "小时" + todou1(m) + "分" + todou1(s) + "秒");

    if (leftsecond <= parseInt(60 * 60 * 4)) {
        if (leftsecond < 0) {
            cc.innerHTML = "直播已结束"
            clearInterval(sh);
            $('#zhibo').attr("src", "vedioend.html"); //发布会视频 
        } else {
            $("#divdown1").html("正在直播......");
            if (leftsecond <= parseInt(60 * 60 * 4) && leftsecond > parseInt(60 * 60 * 2)) {
                if ($('#zhibo').attr("src") != "vedio1.html") {
                    $('#zhibo').attr("src", "vedio1.html"); //第一组直播
                }
            }
            else {
                if ($('#zhibo').attr("src") != "vedio2.html") {
                    $('#zhibo').attr("src", "vedio2.html"); //第二组直播
                }
            }
        }
    }
